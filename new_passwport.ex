defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with  {:ok, time} <- enter_building(now),
          {:ok, manual} <- find_counter_information(now),
          cont <- manual.(birthday),
          {:ok, checksum} <- stamp_form(time,cont,form),
          num_passport <- get_new_passport_number(time,cont,checksum) do
          {:ok, num_passport}
    else
      {:coffee_break, message} ->
